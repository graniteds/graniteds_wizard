                ...
                if (i+1 < length) {
                    ...
                    if (c == last) { // "<%" (template text) or "%>" (gsp tag content)
                        if (inText) {
                            ...
                            // "<%--": lookahead for comment end (we must skip inner "%>" occurrences)
                            if (i+3 < length && template.charAt(i+2) == '-' && template.charAt(i+3) == '-') {
                            	int iEndComment = template.indexOf("--%>", i+4);
                            	if (iEndComment != -1) {
                            		addScriptingElement(i, template.substring(i+2, iEndComment+2));
                            		inText = false;
                            		i = iEndComment+3;
                            	}
                            }
